...
class LiquidEther {
    ...
    init() {
        ...
        // Initialize WebGL
        this.container.appendChild(this.Common.renderer.domElement);
...
class Simulation {
    ...
    getFloatType() {
        // Simple check for float type support
    ...
    createShaderPass() {
        const common = this.parent.Common;
    ...
    calcSize() {
        const width = Math.max(1, Math.round(this.options.resolution * this.parent.Common.width));
        const height = Math.max(1, Math.round(this.options.resolution * this.parent.Common.height));
...
class Output {
    ...
    init() {
        this.simulation = new Simulation(this.parent, this.parent.options);
        this.scene = new THREE.Scene();
        this.camera = new THREE.Camera();

        const paletteTex = this.makePaletteTexture(this.parent.options.colors);
        const bgVec4 = new THREE.Vector4(0, 0, 0, 0);
        if (this.parent.options.bgColor) {
            const c = new THREE.Color(this.parent.options.bgColor);
            bgVec4.set(c.r, c.g, c.b, 0);
        }
    ...
    update() {
        ...
        this.parent.Common.renderer.setRenderTarget(null);
        this.parent.Common.renderer.render(this.scene, this.camera);
    }
}
